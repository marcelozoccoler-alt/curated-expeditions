...
import { laponiaSignature } from "@/lib/signatureTrips";

const DOMAIN = CONTACT.domain.replace(/\/$/, "");
const PATH = "/grupos/laponia-circulo-polar-2027";

const whatsappParams = {
  type: "Roteiro" as const,
  name: "Create Signature — Lapônia: Luzes do Círculo Polar (25/03/2027)",
};
...
const experiences = [
  ...
  { title: "Duas noites sob a aurora", text: "Em Tromsø, duas saídas noturnas acompanham as melhores condições do céu. A natureza não oferece garantias, mas o roteiro amplia as oportunidades de observação." },
...
const itinerary = [
  ...
  { day: "Dia 5 · 29/03", title: "Rovaniemi e a floresta branca", text: "Visita à Aldeia do Papai Noel, travessia do Círculo Polar, condução orientada de snowmobile e pesca em lago congelado, com refeição quente durante a experiência." },
  { day: "Dia 6 · 30/03", title: "Kemi e o mar congelado", text: "Excursão ao SnowExperience365 e embarque em um quebra-gelo. Quando as condições permitirem, a experiência inclui flutuação com traje de proteção nas águas geladas do Báltico." },
  { day: "Dia 7 · 31/03", title: "Rovaniemi → Tromsø", text: "Voo para a Lapônia norueguesa. Visita panorâmica de Tromsø, passagem pela Catedral do Ártico e subida de teleférico ao mirante Storsteinen." },
  { day: "Dia 8 · 01/04", title: "Renas, cultura Sami e aurora", text: "Passeio de trenó puxado por renas e encontro cultural em acampamento Sami. À noite, primeira busca pela aurora em área escolhida conforme o céu." },
  { day: "Dia 9 · 02/04", title: "Huskies e uma nova noite de céu aberto", text: "Safari de trenó com huskies, contato com os cães e pausa para bebida quente. Mais tarde, segunda saída dedicada às luzes do norte." },
  { day: "Dia 10 · 03/04", title: "Tromsø → Oslo", text: "Voo para Oslo, traslado e restante do dia livre para descobrir a capital entre o fiorde e as colinas arborizadas." },
  { day: "Dia 11 · 04/04", title: "Oslo e a memória das expedições", text: "Visita ao Museu Fram, centro histórico e Parque Frogner, onde as esculturas de Gustav Vigeland transformam os ciclos da vida em paisagem." },
  { day: "Dia 12 · 05/04", title: "Oslo → São Paulo", text: "Manhã livre e traslado ao aeroporto para o voo de retorno, com conexão europeia." },
  { day: "Dia 13 · 06/04", title: "Chegada ao Brasil", text: "Desembarque em São Paulo e encerramento de uma travessia pela luz, pelo gelo e pelas culturas do extremo norte." },
];
...
const faqs = [
  { q: "Quando será a viagem à Lapônia?", a: "A saída está prevista para 25 de março de 2027, com retorno ao Brasil em 6 de abril. São 13 dias e 10 noites, sujeitos à confirmação final dos serviços aéreos e terrestres." },
  ...
  { q: "Qual é o investimento da viagem?", a: "O programa aéreo + terrestre sai de € 9.811 por pessoa em apartamento duplo, com opção somente terrestre a partir de € 8.521. Há suplemento para apartamento individual e condições especiais para triplo e crianças até 12 anos. Os valores não incluem taxas de embarque e IOF, que aparecem detalhados na seção Investimento. A Create Travel confirma disponibilidade e a proposta atualizada antes da reserva." },
];

const hotelJsonLd = buildHotelsJsonLd(hotels, laponiaSignature.title, PATH);
const hotelFaqs = buildHotelsFAQs(hotels, laponiaSignature.title, PATH);
...
const jsonLd = [
  {
    ...
    "@type": "TouristTrip",
    "@id": `${DOMAIN}${PATH}#trip`,
    name: laponiaSignature.title,
    url: `${DOMAIN}${PATH}`,
    description: "Jornada Create Signature de 13 dias pela Finlândia e Noruega, com Helsinque, Lapônia, quebra-gelo, cultura Sami, Tromsø, aurora boreal e Oslo.",
    image: laponiaSignature.image,
  ...
  { "@context": "https://schema.org", "@type": "BreadcrumbList", itemListElement: [
    ...
    { "@type": "ListItem", position: 3, name: laponiaSignature.title, item: `${DOMAIN}${PATH}` },
  ] },
  ...hotelJsonLd,
];

const GrupoLaponiaCirculoPolar2027 = () => (
  <div className="min-h-screen bg-background">
    <SEO
      title="Lapônia 2027: Aurora e Círculo Polar Ártico"
      description="Viagem Create Signature de 13 dias à Lapônia com guia brasileiro: aurora boreal, quebra-gelo, snowmobile, cultura Sami, Tromsø, Helsinque e Oslo."
      canonicalPath={PATH}
      ogImage={laponiaSignature.image}
      ogType="article"
      keywords={`viagem Lapônia 2027, Lapônia com guia brasileiro, aurora boreal Tromsø, Círculo Polar Ártico, quebra-gelo Finlândia, viagem de luxo Lapônia, ${hotelKeywords}`}
      jsonLd={jsonLd}
    />
    <Header />
    ...
    <main>
      <section className="relative min-h-[92vh] flex items-end overflow-hidden">
        <img src={laponiaSignature.image} alt={laponiaSignature.imageAlt} className="absolute inset-0 h-full w-full object-cover" width={1600} height={1000} fetchPriority="high" />
        ...
        <div className="container-editorial relative z-10 pb-14 pt-32 md:pb-20">
          <div className="max-w-4xl text-primary-foreground">
            <div className="mb-7 text-primary-foreground/70"><Breadcrumbs items={[{ label: "Create Signature", href: "/create-signature" }, { label: "Lapônia 2027" }]} /></div>
            ...
            <h1 className="heading-hero mb-5">Lapônia: Luzes do Círculo Polar</h1>
            <p className="text-xl md:text-2xl max-w-3xl text-primary-foreground/90 font-light leading-relaxed mb-8">
              Uma travessia pelo silêncio branco do norte, onde o mar se transforma em gelo e o céu, quando decide se abrir, escreve luz sobre a noite.
            </p>
      ...
      <section className="section-padding">
        <div className="container-editorial grid lg:grid-cols-[1.15fr_0.85fr] gap-10 items-start">
          ...
          <aside className="bg-muted border border-border rounded-lg p-8 md:p-10">
            ...
            <p className="text-caption text-muted-foreground mb-2">Por pessoa, em euro, em apartamento duplo · Saída de 25 de março</p>
            <h2 className="font-serif text-4xl mb-1">€ 9.811</h2>
            ...
            <dl className="space-y-3 text-sm border-t border-border pt-6 mb-6">
              <div className="flex justify-between gap-4"><dt className="text-muted-foreground">Somente terrestre (apto. duplo)</dt><dd className="font-medium">€ 8.521</dd></div>
              <div className="flex justify-between gap-4"><dt className="text-muted-foreground">Suplemento apto. individual</dt><dd className="font-medium">€ 1.867</dd></div>
              <div className="flex justify-between gap-4"><dt className="text-muted-foreground">Redução apto. triplo</dt><dd className="font-medium">€ 353</dd></div>
              <div className="flex justify-between gap-4"><dt className="text-muted-foreground">Redução menor de 12 anos compartilhando apto. duplo com 2 adultos</dt><dd className="font-medium">€ 823</dd></div>
            </dl>
            <p className="text-caption text-muted-foreground mb-3">Taxas e impostos (IOF e taxa de embarque)</p>
            <dl className="space-y-3 text-sm mb-6">
              <div className="flex justify-between gap-4"><dt className="text-muted-foreground">Aéreo + terrestre · apto. duplo</dt><dd className="font-medium">€ 470</dd></div>
              <div className="flex justify-between gap-4"><dt className="text-muted-foreground">Aéreo + terrestre · apto. individual</dt><dd className="font-medium">€ 517</dd></div>
              <div className="flex justify-between gap-4"><dt className="text-muted-foreground">Aéreo + terrestre · criança (menor de 12 anos)</dt><dd className="font-medium">€ 450</dd></div>
              <div className="flex justify-between gap-4"><dt className="text-muted-foreground">Terrestre · apto. duplo</dt><dd className="font-medium">€ 294</dd></div>
              <div className="flex justify-between gap-4"><dt className="text-muted-foreground">Terrestre · apto. individual</dt><dd className="font-medium">€ 341</dd></div>
              <div className="flex justify-between gap-4"><dt className="text-muted-foreground">Terrestre · criança (menor de 12 anos)</dt><dd className="font-medium">€ 274</dd></div>
            </dl>
...
export default GrupoLaponiaCirculoPolar2027;