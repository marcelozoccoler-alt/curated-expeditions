...
import heroImg from "@/assets/grupo-lagos-chilenos-julho-2027.jpg";

const CANONICAL = "/grupos/lagos-chilenos-julho-2027";
const GROUP_NAME = "Grupo Lagos Chilenos · Julho 2027 — Create Travel";
const whatsappParams = {
  type: "Roteiro" as const,
  name: "Grupo Lagos Chilenos · Julho 2027 (saída 14/07/2027)",
};

const benefits = [
  { title: "Guia acompanhante desde o Brasil", desc: "Encontro em Guarulhos e acompanhamento durante todo o circuito até o retorno." },
  { title: "Inverno patagônico", desc: "Vulcões nevados, lagos cristalinos e cidades de herança alemã no período mais dramático e fotogênico do sul do Chile." },
  { title: "Santiago e vinhos", desc: "City tour pela capital aos pés dos Andes e visita com degustação na tradicional Vinícola Undurraga, no Vale do Maipo." },
  ...
  { title: "Puerto Varas e Frutillar", desc: "O Lago Llanquihue, as Cataratas de Petrohué e o charme germânico de Frutillar, com o Teatro del Lago." },
...
const itinerary = [
  ...
  { day: "Dia 5 — 18/07 (dom)", title: "Puerto Varas · Puerto Montt", points: ["Cerro Philippi, Igreja do Sagrado Coração de Jesus e casas históricas de estilo alemão", "Orla do Lago Llanquihue até Puerto Chico", "Puerto Montt: Plaza de Armas, Catedral, Monumento aos Colonos Alemães e mirante sobre a Baía de Reloncaví", "Enseada de Angelmó, com mercado de frutos do mar e feira de artesanato"] },
  { day: "Dia 6 — 19/07 (seg)", title: "Puerto Varas · Peulla", points: ["Estrada cênica margeando o Lago Llanquihue até Ensenada, aos pés do Vulcão Osorno", "Navegação pelo Lago Todos los Santos, o Lago Esmeralda, até Peulla", "Almoço livre (não incluído) e tarde livre na vila ecoturística", "Retorno navegando até Petrohué"] },
  { day: "Dia 7 — 20/07 (ter)", title: "Puerto Varas · Petrohué · Frutillar", points: ["Parque Nacional Vicente Pérez Rosales e as Cataratas de Petrohué", "Parada para almoço em Ensenada (não incluído), com vista para o lago e os vulcões", "Frutillar: orla do Lago Llanquihue, Teatro del Lago, cafés e lojas de artesanato"] },
  { day: "Dia 8 — 21/07 (qua)", title: "Puerto Varas → Puerto Montt → Santiago", points: ["Apartamentos disponíveis até às 11h e traslado ao aeroporto", "Voo LATAM 284 às 18h22, chegada a Santiago às 20h05", "Procedimentos de conexão"] },
  { day: "Dia 9 — 22/07 (qui)", title: "Santiago → São Paulo", points: ["Embarque no voo LATAM 8097 às 00h20", "Chegada prevista a Guarulhos às 05h30 · fim dos nossos serviços"] },
];
...
const included = [
  ...
  "Visitas com guia local em português ou espanhol conforme o roteiro (passeio a Petrohué em serviço regular, com guia em espanhol)",
  "Entradas: Vinícola Undurraga com degustação em Santiago e Parque Nacional Vicente Pérez Rosales na região dos lagos",
...
const notIncluded = [
  ...
  "Almoços indicados como não incluídos no roteiro",
...
const faqs = [
  ...
  { q: "Quanto frio faz em julho na Região dos Lagos?", a: "É pleno inverno patagônico: em Puerto Varas e Puerto Montt as temperaturas ficam em geral entre 2°C e 10°C, com possibilidade de chuva e neve nas montanhas. Leve casaco térmico, segunda pele, luvas, gorro, cachecol e calçado impermeável." },
  { q: "O passeio a Valparaíso e Viña del Mar está incluído?", a: "Não. É um opcional oferecido no dia livre em Santiago, contratado à parte, assim como o almoço à beira-mar durante o passeio." },
...
const jsonLd = {
  ...
  name: GROUP_NAME,
  description: "Viagem de 9 dias pelos Lagos Chilenos em pleno inverno patagônico, de 14 a 22 de julho de 2027, com guia acompanhante desde o Brasil: Santiago, Vinícola Undurraga, Puerto Varas, Peulla, Petrohué e Frutillar.",
  image: heroImg,
  touristType: "Natureza, lagos, vulcões, vinhos e inverno patagônico",
  itinerary: { "@type": "ItemList", itemListElement: itinerary.map((item, index) => ({ "@type": "ListItem", position: index + 1, name: `${item.day} — ${item.title}` })) },
  offers: { "@type": "Offer", price: "2898.00", priceCurrency: "USD", availability: "https://schema.org/LimitedAvailability", validThrough: "2027-07-14", priceValidUntil: "2027-07-14" },
  provider: { "@id": "https://createtravel.tur.br/#organization" },
};

const GrupoLagosChilenosJulho2027 = () => {
  const ctaLink = generateWhatsAppLink(whatsappParams);
  return (
    <div className="min-h-screen">
      <SEO title="Lagos Chilenos Julho 2027 com Guia desde o Brasil | Create" description="9 dias em pleno inverno patagônico: Santiago, Vinícola Undurraga, Puerto Varas, Peulla, Petrohué e Frutillar. Saída 14/07/2027 com guia desde o Brasil." canonicalPath={CANONICAL} ogImage={heroImg} ogType="article" jsonLd={[jsonLd, ...hotelsJsonLd]} keywords={`Lagos Chilenos julho 2027, viagem em grupo Chile, Puerto Varas com guia brasileiro, excursão Chile saindo de São Paulo, LATAM, Peulla, Frutillar, inverno patagônico, ${hotelsKeywords}`} />
      <Header />
      <WhatsAppButton variant="float" params={whatsappParams} />

      <section className="relative h-[92vh] flex items-center overflow-hidden">
        <div className="absolute inset-0"><img src={heroImg} alt="Vulcão nevado refletido em lago na Região dos Lagos Chilenos, em pleno inverno" className="w-full h-full object-cover" width={1600} height={900} fetchPriority="high" /><div className="absolute inset-0 hero-overlay" /></div>
        <div className="container-editorial relative z-10"><motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="max-w-3xl">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gold/90 text-navy text-xs font-semibold uppercase tracking-wider mb-6"><Sparkles size={14} /> Inverno patagônico 2027 · Lugares limitados</div>
          <p className="text-caption text-gold mb-4">Grupo com guia desde o Brasil · 14 a 22 de julho de 2027</p>
          <h1 className="heading-hero text-white mb-6">Lagos Chilenos: a magia do inverno patagônico</h1>
          <p className="text-xl md:text-2xl text-white/90 mb-8 font-light">9 dias entre vulcões nevados, lagos cristalinos, vinícolas e o charme alemão de Frutillar — com guia acompanhante desde o Brasil.</p>
      ...
      <section className="bg-muted py-10 border-b border-border"><div className="container-editorial grid grid-cols-2 md:grid-cols-4 gap-6">{[
        { icon: Calendar, label: "Datas", value: "14 a 22/07/2027" }, { icon: Users, label: "Perfil", value: "Guia desde o Brasil" }, { icon: Plane, label: "Voos", value: "LATAM" }, { icon: Hotel, label: "Hospedagem", value: "Hotéis 1ª categoria" },
      ...
      <section id="investimento" className="section-padding bg-gradient-hero text-white"><div className="container-editorial max-w-4xl text-center"><p className="text-caption text-gold mb-4">Investimento</p><h2 className="heading-section mb-6">A partir de US$ 2.865 por pessoa</h2><p className="text-white/85 mb-8">Valores por pessoa, acrescidos de IOF e de USD 160 em taxas de aeroporto e combustível. Sujeitos a disponibilidade e confirmação aérea.</p><div className="grid sm:grid-cols-3 gap-4 mb-8 text-left">{[
        { label: "Apto triplo", price: "US$ 2.865", extra: "+ IOF US$ 73" }, { label: "Apto duplo", price: "US$ 2.898", extra: "+ IOF US$ 74" }, { label: "Apto individual", price: "US$ 3.670", extra: "+ IOF US$ 99" },
      ...
      <section className="section-padding"><div className="container-editorial text-center max-w-2xl"><h2 className="heading-section text-foreground mb-6">Lugares limitados — grupo com guia desde o Brasil</h2><p className="text-muted-foreground mb-8">Fale com a curadoria Create Travel e consulte a disponibilidade para os Lagos Chilenos em julho de 2027.</p><div className="flex flex-col sm:flex-row items-center justify-center gap-4"><a href={ctaLink} target="_blank" rel="noopener noreferrer" className="btn-whatsapp text-lg px-8 py-4">Falar no WhatsApp</a><Link to="/embarque-com-a-create" className="btn-outline">Ver outras saídas <ArrowRight size={18} /></Link></div></div></section>
      <Footer />
    </div>
  );
};

export default GrupoLagosChilenosJulho2027;
