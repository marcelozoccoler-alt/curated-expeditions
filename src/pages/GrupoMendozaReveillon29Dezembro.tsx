...
const CANONICAL = "/grupos/mendoza-reveillon-29-dezembro-2026";
const GROUP_NAME = "Grupo Mendoza · Réveillon 2027 (saída 29/12) — Create Travel";
const whatsappParams = {
  type: "Roteiro" as const,
  name: "Grupo Mendoza · Réveillon 2027 (saída 29/12/2026)",
};

const benefits = [
  { title: "Guia acompanhante desde o Brasil", desc: "Encontro em Guarulhos e acompanhamento durante toda a viagem até o retorno." },
  { title: "Réveillon com show folclórico", desc: "Jantar festivo e programação de virada no Ahijuna Show Folclórico, a uma quadra do hotel." },
  ...
  { title: "Voos diretos pela GOL", desc: "São Paulo–Mendoza em classe econômica, com uma mala de 23 kg." },
];
...
const itinerary = [
  ...
  { day: "Dia 5 — 02/01 (sáb)", title: "Vale de Uco", points: ["Saída para o Vale de Uco, a cerca de 100 km da cidade", "Visita a três vinícolas, com tours e degustações", "Almoço incluído de três passos harmonizado em uma das bodegas", "Retorno ao hotel"] },
  { day: "Dia 6 — 03/01 (dom)", title: "Mendoza → São Paulo", points: ["Apartamentos disponíveis até às 12h e traslado ao aeroporto", "Voo GOL 7487 às 14h20", "Chegada prevista a Guarulhos às 17h20 · fim dos nossos serviços"] },
];
...
const included = [
  ...
  "Visitas com guia local em português ou espanhol: Vinícola Santa Julia, Tour Alta Montanha, city tour em Mendoza e Vale de Uco com três vinícolas",
...
const faqs = [
  ...
  { q: "Como é a virada do ano?", a: "A programação de Réveillon acontece no Ahijuna Show Folclórico, com jantar festivo, vinhos mendocinos e apresentações de música e danças típicas. O local fica a uma quadra do hotel e o deslocamento é a pé, pois não inclui traslados." },
...
const jsonLd = {
  ...
  name: GROUP_NAME,
  description: "Réveillon 2027 em Mendoza, de 29 de dezembro de 2026 a 03 de janeiro de 2027, com guia desde o Brasil: Vinícola Santa Julia, Alta Montanha e Aconcágua, city tour, Vale de Uco e virada com show folclórico.",
  image: heroImg,
  ...
  offers: { "@type": "Offer", price: "2798.00", priceCurrency: "USD", availability: "https://schema.org/LimitedAvailability", validThrough: "2026-12-29", priceValidUntil: "2026-12-29" },
  provider: { "@id": "https://createtravel.tur.br/#organization" },
};

const GrupoMendozaReveillon29Dezembro = () => {
  const ctaLink = generateWhatsAppLink(whatsappParams);
  return (
    <div className="min-h-screen">
      <SEO title="Réveillon 2027 em Mendoza com Guia desde o Brasil | Create" description="6 dias entre vinhedos e Andes: Santa Julia, Alta Montanha e Aconcágua, Vale de Uco e virada com show folclórico. Saída 29/12/2026 com guia desde o Brasil." canonicalPath={CANONICAL} ogImage={heroImg} ogType="article" jsonLd={[jsonLd, ...hotelsJsonLd]} keywords={`Réveillon Mendoza 2027, viagem em grupo Mendoza, Vale de Uco com guia brasileiro, Aconcágua, vinícolas argentinas, GOL, ${hotelsKeywords}`} />
      <Header />
      <WhatsAppButton variant="float" params={whatsappParams} />

      <section className="relative h-[92vh] flex items-center overflow-hidden">
        <div className="absolute inset-0"><img src={heroImg} alt="Vinhedos de Mendoza ao pôr do sol com a Cordilheira dos Andes nevada ao fundo" className="w-full h-full object-cover" width={1600} height={900} fetchPriority="high" /><div className="absolute inset-0 hero-overlay" /></div>
        <div className="container-editorial relative z-10"><motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="max-w-3xl">
          ...
          <p className="text-caption text-gold mb-4">Grupo com guia desde o Brasil · 29/12/2026 a 03/01/2027</p>
          <h1 className="heading-hero text-white mb-6">Réveillon em Mendoza: entre vinhos e Andes</h1>
          <p className="text-xl md:text-2xl text-white/90 mb-8 font-light">6 dias de bodegas, alta montanha, Vale de Uco e uma virada de ano com gastronomia e show folclórico argentino.</p>
      ...
      <section className="bg-muted py-10 border-b border-border"><div className="container-editorial grid grid-cols-2 md:grid-cols-4 gap-6">{[
        { icon: Calendar, label: "Datas", value: "29/12/2026 a 03/01/2027" }, { icon: Users, label: "Perfil", value: "Guia desde o Brasil" }, { icon: Plane, label: "Voos", value: "GOL" }, { icon: Hotel, label: "Hospedagem", value: "Hotéis 1ª categoria" },
      ...
      <section className="section-padding"><div className="container-editorial"><div className="text-center mb-14 max-w-2xl mx-auto"><p className="text-caption text-gold mb-4">Por que viver esta jornada</p><h2 className="heading-section text-foreground">Um ano novo aos pés da Cordilheira</h2></div><div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">{benefits.map((benefit, index) => <motion.div key={benefit.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.05 }} className="bg-card p-8 rounded-xl shadow-card border border-border"><div className="w-10 h-10 rounded-full bg-gold/15 flex items-center justify-center mb-4"><Check className="text-gold" size={20} /></div><h3 className="text-lg font-serif font-semibold mb-2">{benefit.title}</h3><p className="text-muted-foreground text-sm leading-relaxed">{benefit.desc}</p></motion.div>)}</div></div></section>

      <section className="section-padding bg-navy text-white"><div className="container-editorial grid lg:grid-cols-2 gap-12 items-start"><div><p className="text-caption text-gold mb-4">Passagens aéreas incluídas</p><h2 className="heading-section mb-6">Voos pela GOL</h2><p className="text-white/85 leading-relaxed">Classe econômica, com uma mala despachada de até 23 kg por pessoa, em voos diretos. Horários previstos, sujeitos a ajustes da companhia aérea.</p></div><div className="space-y-4"><div className="bg-white/5 border border-white/10 rounded-xl p-6"><p className="text-xs uppercase tracking-wider text-gold mb-2">Ida</p><p className="font-serif text-lg">São Paulo (GRU) → Mendoza (MDZ)</p><p className="text-white/75 text-sm mt-1">29/12 · G3 7486 às 09h15, chegada às 13h30</p></div><div className="bg-white/5 border border-white/10 rounded-xl p-6"><p className="text-xs uppercase tracking-wider text-gold mb-2">Volta</p><p className="font-serif text-lg">Mendoza (MDZ) → São Paulo (GRU)</p><p className="text-white/75 text-sm mt-1">03/01 · G3 7487 às 14h20, chegada às 17h20</p></div></div></div></section>
      ...
      <section id="investimento" className="section-padding bg-gradient-hero text-white"><div className="container-editorial max-w-4xl text-center"><p className="text-caption text-gold mb-4">Investimento</p><h2 className="heading-section mb-6">A partir de US$ 2.778 por pessoa</h2><p className="text-white/85 mb-8">Valores por pessoa, acrescidos de IOF e de USD 168 em taxas de aeroporto e combustível. Sujeitos a disponibilidade e confirmação aérea.</p><div className="grid sm:grid-cols-3 gap-4 mb-8 text-left">{[
        { label: "Apto triplo", price: "US$ 2.778", extra: "+ IOF US$ 65" }, { label: "Apto duplo", price: "US$ 2.798", extra: "+ IOF US$ 66" }, { label: "Apto individual", price: "US$ 3.219", extra: "+ IOF US$ 79" },
...
export default GrupoMendozaReveillon29Dezembro;
