...
const CANONICAL = "/grupos/lagos-chilenos-maio-2027";
const GROUP_NAME = "Grupo Lagos Chilenos · Maio 2027 — Create Travel";
const whatsappParams = {
  type: "Roteiro" as const,
  name: "Grupo Lagos Chilenos · Maio 2027 (saída 18/05/2027)",
};

const benefits = [
  { title: "Guia acompanhante desde o Brasil", desc: "Encontro em Guarulhos e acompanhamento durante todo o circuito até o retorno." },
  { title: "Outono chileno", desc: "Folhagens douradas e avermelhadas cobrindo montanhas e vilarejos, com temperaturas suaves." },
...
const itinerary = [
  ...
  { day: "Dia 5 — 22/05 (sáb)", title: "Puerto Varas e Puerto Montt", points: ["Cerro Philippi, Igreja do Sagrado Coração de Jesus e casas históricas de estilo alemão", "Orla do Lago Llanquihue até Puerto Chico", "Puerto Montt: Plaza de Armas, Catedral, Monumento aos Colonos Alemães e mirante sobre a Baía de Reloncaví", "Enseada de Angelmó, com mercado de frutos do mar e feira de artesanato"] },
  { day: "Dia 6 — 23/05 (dom)", title: "Puerto Varas · Peulla", points: ["Estrada cênica margeando o Lago Llanquihue até Ensenada, aos pés do Vulcão Osorno", "Navegação pelo Lago Todos los Santos, o Lago Esmeralda, até Peulla", "Almoço livre (não incluso) e tarde livre na vila ecoturística", "Retorno navegando até Petrohué"] },
  { day: "Dia 7 — 24/05 (seg)", title: "Petrohué e Frutillar", points: ["Parque Nacional Vicente Pérez Rosales e as Cataratas de Petrohué", "Parada para almoço em Ensenada (não incluso), com vista para o lago e os vulcões", "Frutillar: orla do Lago Llanquihue, Teatro del Lago, cafés e lojas de artesanato"] },
  { day: "Dia 8 — 25/05 (ter)", title: "Puerto Varas → Santiago → São Paulo", points: ["Apartamentos disponíveis até às 11h e traslado ao aeroporto", "Voo LATAM 256 às 14h14, chegada a Santiago às 15h57", "Conexão no voo LATAM 8097 às 18h50, chegada a Guarulhos às 23h45 · fim dos nossos serviços"] },
];
...
const faqs = [
  ...
  { q: "Como é o clima em maio na Região dos Lagos?", a: "É pleno outono: temperaturas em geral entre 5°C e 15°C, com chuvas ocasionais e folhagens douradas. Leve casaco impermeável, segunda pele, cachecol e calçado fechado confortável." },
...
const jsonLd = {
  ...
  name: GROUP_NAME,
  description: "Viagem de 8 dias pelos Lagos Chilenos em pleno outono, de 18 a 25 de maio de 2027, com guia acompanhante desde o Brasil: Santiago, Vinícola Undurraga, Puerto Varas, Peulla, Petrohué e Frutillar.",
  image: heroImg,
  touristType: "Natureza, lagos, vulcões, vinhos e outono no sul do Chile",
  itinerary: { "@type": "ItemList", itemListElement: itinerary.map((item, index) => ({ "@type": "ListItem", position: index + 1, name: `${item.day} — ${item.title}` })) },
  offers: { "@type": "Offer", price: "2698.00", priceCurrency: "USD", availability: "https://schema.org/LimitedAvailability", validThrough: "2027-05-18", priceValidUntil: "2027-05-18" },
  provider: { "@id": "https://createtravel.tur.br/#organization" },
};

const GrupoLagosChilenosMaio2027 = () => {
  const ctaLink = generateWhatsAppLink(whatsappParams);
  return (
    <div className="min-h-screen">
      <SEO title="Lagos Chilenos Maio 2027 com Guia desde o Brasil | Create" description="8 dias no outono chileno: Santiago, Vinícola Undurraga, Puerto Varas, Peulla, Petrohué e Frutillar. Saída 18/05/2027 com guia desde o Brasil." canonicalPath={CANONICAL} ogImage={heroImg} ogType="article" jsonLd={[jsonLd, ...hotelsJsonLd]} keywords={`Lagos Chilenos maio 2027, viagem em grupo Chile, Puerto Varas com guia brasileiro, outono no sul do Chile, LATAM, Peulla, Frutillar, ${hotelsKeywords}`} />
      <Header />
      ...
      <section className="relative h-[92vh] flex items-center overflow-hidden">
        ...
        <div className="container-editorial relative z-10"><motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="max-w-3xl">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gold/90 text-navy text-xs font-semibold uppercase tracking-wider mb-6"><Sparkles size={14} /> Outono 2027 · Lugares limitados</div>
          <p className="text-caption text-gold mb-4">Grupo com guia desde o Brasil · 18 a 25 de maio de 2027</p>
          <h1 className="heading-hero text-white mb-6">Lagos Chilenos: o sul do Chile em pleno outono</h1>
          <p className="text-xl md:text-2xl text-white/90 mb-8 font-light">8 dias entre vulcões, lagos cristalinos, vinhos e cidades de herança alemã — com guia acompanhante desde o Brasil.</p>
      ...
      <section className="bg-muted py-10 border-b border-border"><div className="container-editorial grid grid-cols-2 md:grid-cols-4 gap-6">{[
        { icon: Calendar, label: "Datas", value: "18 a 25/05/2027" }, { icon: Users, label: "Perfil", value: "Guia desde o Brasil" }, { icon: Plane, label: "Voos", value: "LATAM" }, { icon: Hotel, label: "Hospedagem", value: "Hotéis 1ª categoria" },
      ...
      <section className="section-padding"><div className="container-editorial text-center max-w-2xl"><h2 className="heading-section text-foreground mb-6">Lugares limitados — grupo com guia desde o Brasil</h2><p className="text-muted-foreground mb-8">Fale com a curadoria Create Travel e consulte a disponibilidade para os Lagos Chilenos em maio de 2027.</p><div className="flex flex-col sm:flex-row items-center justify-center gap-4"><a href={ctaLink} target="_blank" rel="noopener noreferrer" className="btn-whatsapp text-lg px-8 py-4">Falar no WhatsApp</a><Link to="/embarque-com-a-create" className="btn-outline">Ver outras saídas <ArrowRight size={18} /></Link></div></div></section>
      <Footer />
    </div>
  );
};

export default GrupoLagosChilenosMaio2027;
